...
Patient booking API — solo, personal/learning project. Governed by dotnet-Codex-kit
(`.Codex/rules/*.md`, `.Codex/skills/*`, `.Codex/agents/*`). This file is the canonical
...
These instructions apply to Codex, Codex, and any other coding agent. Reuse the Markdown
under `.Codex/rules/`, `.Codex/skills/`, `.Codex/agents/`, and `.Codex/knowledge/` directly;
...
- Codex uses the existing `.Codex` settings, hooks, agents, skills, and slash-command wiring.
...
- Codex-only frontmatter, model aliases, memory paths, worktree isolation, tool names, hook
...
- Codex model-selection guidance lives in `.Codex/rules/agents.md`. Codex should ignore
  Codex-specific model aliases there and use the current Codex session configuration.

### Agent roster

| Agent | Canonical playbook | Primary domain |
|---|---|---|
| dotnet-architect | `.Codex/agents/dotnet-architect.md` | Architecture, project structure, module boundaries |
| api-designer | `.Codex/agents/api-designer.md` | APIs, OpenAPI, versioning, rate limiting |
| ef-core-specialist | `.Codex/agents/ef-core-specialist.md` | Database, queries, migrations, EF Core patterns |
| test-engineer | `.Codex/agents/test-engineer.md` | Test strategy, xUnit, WebApplicationFactory, Testcontainers |
| security-auditor | `.Codex/agents/security-auditor.md` | Authentication, authorization, OWASP, secrets |
| performance-analyst | `.Codex/agents/performance-analyst.md` | Benchmarks, memory, async patterns, caching |
| devops-engineer | `.Codex/agents/devops-engineer.md` | Docker, CI/CD, Aspire, deployment |
| code-reviewer | `.Codex/agents/code-reviewer.md` | Multi-dimensional code review |
| build-error-resolver | `.Codex/agents/build-error-resolver.md` | Autonomous build-error fixing |
| refactor-cleaner | `.Codex/agents/refactor-cleaner.md` | Systematic dead-code removal and cleanup |
...
1. Read `.Codex/skills/modern-csharp/SKILL.md` for all .NET work.
...
- `wrap-up`: session handoff lifecycle using `.Codex/handoff.md`.
...
The following names identify the shared kit workflows. Codex may expose them as slash
...
**Known conflict with `.Codex/rules/architecture.md` — left unresolved on purpose.**
...
this project. This file (`AGENTS.md`) is the source of truth for this project's actual choice —
...
design (see `.Codex/dotnet-Codex-kit-SOURCE.md`).
