...
description: >
  ...
  completed work, pending tasks, and learnings into .Codex/handoff.md, and the
...
- **Session END** — capture exactly three things: what was DONE, what is
  PENDING, and what was LEARNED. Write them to `.Codex/handoff.md` and flow
...
4. **Extract learnings** — Corrections and discoveries worth keeping go to
   ...
   `.Codex/instincts.md` (via `instinct-system`). The handoff's Learned
...
1. **Load context** — Read `.Codex/handoff.md` (pending work), `MEMORY.md`
   (permanent rules), `.Codex/instincts.md` (patterns at 0.7+ confidence).
