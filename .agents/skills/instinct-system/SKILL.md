...
description: >
  ...
  (observe-hypothesize-confirm, stored in .Codex/instincts.md), user
  corrections captured as permanent rules in MEMORY.md, and organic
  discoveries logged to .Codex/learning-log.md. Includes status, export,
...
One learning system, three tiers. Every signal Codex receives during work — an observed pattern, a user correction, a surprising discovery — routes to exactly one store.

## Core Principles

1. **Three tiers, one routing decision** — *Instincts* are unconfirmed hypotheses (`.Codex/instincts.md`). *Corrections* are user-confirmed rules (`MEMORY.md`). *Discoveries* are insights that explain the world (`.Codex/learning-log.md`). Rules prescribe behavior; insights describe it; instincts are rules-in-waiting. Never mix the tiers — a hypothesis in MEMORY.md pollutes permanent knowledge, and a confirmed rule left as an instinct gets forgotten.
...
|--------|-------------|----------|
| Pattern observed, not yet confirmed ("I think they always...") | `.Codex/instincts.md` at 0.3 | Until promoted or discarded |
...
| Non-obvious discovery (bug root cause, gotcha, workaround, perf finding) | `.Codex/learning-log.md` | 3–6 months, then archive or promote |
...
2. HYPOTHESIZE — Write to .Codex/instincts.md at confidence 0.3:
...
`.Codex/instincts.md`, grouped by category with structured metadata:
...
Log organic findings to `.Codex/learning-log.md` the moment they occur — a 2-line entry written immediately beats a paragraph reconstructed later. Entry format:
...
2. Read .Codex/instincts.md — load 0.7+ as defaults, note 0.5–0.6
...
1. Read `.Codex/instincts.md`; parse each entry's metadata.
...
1. Read `.Codex/instincts.md`; filter to confidence > 0.7 (threshold configurable).
2. Strip project-specific context (file paths, line numbers) while preserving the pattern itself.
3. Write to `.Codex/instincts-export.md` with portable metadata.
...
1. Read the export file (user-provided path or `.Codex/instincts-export.md`) and the current `.Codex/instincts.md`.
...
Codex: "Fixed." *Next session: same mistake*

# GOOD — fix AND capture
Codex: "Fixed. Added to Memory > Data Access:
