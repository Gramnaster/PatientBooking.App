...
description: >
  Codex workflow mastery for .NET developers. Covers parallel execution
  ...
  .NET ecosystem.
  Load this skill when setting up Codex for a .NET project, optimizing
  ...
  "permissions", "hooks", "10x", "setup Codex", "speed up development",
  "context", "tokens", "budget", "running out of context", "too many files",
  or "large codebase".
  Inspired by tips from Boris Cherny (creator of Codex) and the Anthropic team.
---
...
1. **Parallel over sequential** — Run 3-5 Codex sessions simultaneously using git worktrees. Build a feature in one, fix a bug in another, run tests in a third. The single biggest productivity unlock.
2. **Plan then execute** — For any non-trivial task, start in plan mode, iterate until the plan is bulletproof, then switch to auto-accept. A good plan means Codex 1-shots the implementation.
3. **Verification closes the loop** — Give Codex a way to prove its work: `dotnet build`, `dotnet test`, `get_diagnostics` via MCP. This single practice 2-3x the quality of the output.
...
6. **Compound your knowledge** — Every correction becomes a rule in `MEMORY.md` (see `instinct-system` skill). Every PR review adds a learning. Over time, Codex's mistake rate drops because your project's knowledge base grows.

## Patterns

### Parallel Sessions with Git Worktrees

The biggest productivity multiplier. Each worktree gets its own Codex session, its own files, zero conflicts.
...
# Start Codex in each (separate terminal tabs)
cd ../my-project-feature && Codex
cd ../my-project-bugfix && Codex
cd ../my-project-tests && Codex
```

**Practical .NET workflow:**

| Worktree | Task | Codex Session |
|----------|------|---------------|
...
Why `|| true`: The hook should never block Codex's workflow. If formatting fails (e.g., on a non-C# file), silently continue.

### Pre-Allow Safe .NET Permissions

Stop clicking "allow" for every `dotnet` command. Add these to `.Codex/settings.json`:
...
Step 5: Codex executes with auto-accept — often 1-shots the implementation
```

**Advanced pattern:** Have one Codex write the plan, then spin up a second Codex session to review it as a staff engineer:
...
Boris's #1 tip: "Give Codex a way to verify its work." The short version: always tell Codex to run `dotnet build`, `dotnet test`, `get_diagnostics`, and `dotnet format --verify-no-changes` before declaring done. The verify skill has the complete pipeline with short-circuit rules and report templates.
...
Create reusable subagents in `.Codex/agents/`:

```markdown
<!-- .Codex/agents/verify-api.md -->
...
```markdown
<!-- .Codex/agents/code-simplifier.md -->
...
The rules in `.Codex/rules/agents.md` already mandate MCP-first navigation (`find_symbol` before file reads, `get_diagnostics` over builds). This section is the strategy layer on top: how to budget, when to offload, and how to recover.
...
*Codex modifies 15 files, misses half the invariants, tangles the migration*
...
Codex: "Here's the implementation" *generic, works but not great*
You: "Looks good, ship it"

// GOOD — push for quality
Codex: "Here's the implementation"
...
| Codex made a mistake | "Update AGENTS.md so you don't make that mistake again" |
