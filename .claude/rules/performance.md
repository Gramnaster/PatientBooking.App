...
- **Framework overhead is not the bottleneck.** ASP.NET Core/Kestrel routing + JSON serialization costs low tens of microseconds per request — see [TechEmpower](https://www.techempower.com/benchmarks/). Everything else in this file is about the I/O-bound work (DB, cache, downstream calls) that actually fills the latency budget.
- **Target p99**: under 100ms for single-entity/list reads (Google SRE's canonical internal-service SLO — see [sre.google](https://sre.google/sre-book/service-level-objectives/)), under 300ms for endpoints joining multiple tables. Past 500ms, treat it as a bug regardless of how much relational complexity "justifies" it.
- **Complexity, not framework overhead, is what grows latency.** Each additional `.Include()` on a *sibling* collection risks cartesian explosion (row-count multiplication), not just extra CPU cost — see the `ef-core` skill's "Split Queries" section for the fix.
