...
Multiple `.Include()` calls for **sibling** collections multiply rows in a single SQL query — an order with 10 items and 5 history entries returns 50 rows (10 × 5), not 15, with every parent column repeated on each one. `AsSplitQuery()` issues one query per collection instead of one JOINed query, so the database returns 15 rows total.

```csharp
// BAD — single query, sibling collections multiply: 10 items × 5 history = 50 rows returned
var order = await db.Orders
    .Include(o => o.Items)
    .Include(o => o.History)
    .FirstOrDefaultAsync(o => o.Id == id, ct);

// GOOD — one query per collection, no row multiplication
var order = await db.Orders
    .Include(o => o.Items)
    .Include(o => o.History)
    .AsSplitQuery()
...
A single `.Include()` with no sibling collection never explodes — only two or more collection navigations at the *same* level do. Split queries trade the JOIN for extra round trips and lose single-query consistency (a concurrent write between the split queries can produce a torn read), so prefer a `.Select()` projection over the entity graph entirely wherever the caller doesn't need change tracking. See [Microsoft's Single vs. Split Queries docs](https://learn.microsoft.com/en-us/ef/core/querying/single-split-queries).
...
| Multiple sibling collection `Include`s | `AsSplitQuery()`, or `Select()` projection instead |
